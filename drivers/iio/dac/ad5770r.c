...
static int ad5770r_channel_config(struct ad5770r_state *st)
{
	...
	unsigned int num, num_channels;
	struct fwnode_handle *child;
	bool ch_config[AD5770R_MAX_CHANNELS] = {0};

	num_channels = of_get_available_child_count(st->spi->dev.of_node);
	if (num_channels < AD5770R_MAX_CHANNELS) {
		dev_err(&st->spi->dev, "not enough channels defined\n");
		return -ENODEV;
	}
